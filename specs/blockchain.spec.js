...
describe('Blockchain', function () {
  ...
  function * invokeChaincodeAndVerify (newuser, chaincodeID, chaincode, useSetFn = false , useQueryResult = false , attrs = null) {
  ...
  function * renewVaultToken (config) {
    if (config.vaultToken != undefined && config.vaultUrl != undefined) {
      var vaultUrl = url.parse(config.vaultUrl)
      var renewUrl = vaultUrl.protocol + '//' + vaultUrl.host
      renewUrl = renewUrl + '/v1/auth/token/renew-self'
      var pPost = Promise.promisify(request.post)
      let vaultHeaders = {
        'X-Vault-Token': config.vaultToken
      }
      try {
        var response = yield pPost({
          url: renewUrl,
  ...
  before(function * () {
    ...
    hyperledgerUtil.setupChain(config.blockchainName, config.ca, config.peers, config.eventhub, config.keyValueLocation, config.vaultUrl, config.vaultToken)
  })

  after(function* () {
    yield* renewVaultToken(config)
    hyperledgerUtil.disconnectEventHub()
  })
  ...
  describe('Regular chaincode', function () {
    ...
    it('should be able to deploy chaincode', function * () {
      if (!config.skipDeploy) {
        this.timeout(60000)
        hyperledgerUtil.setDeployWaitTime(60)
    ...
    it('should reset the chaincode state', function * () {
      this.timeout(15000)
      hyperledgerUtil.setInvokeWaitTime(15)
